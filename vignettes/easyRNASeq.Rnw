...
the \Robject{simpleRNASeq} vignette.
...
Throughout this vignette, the data present in our GitHub repository will be used
to demonstrate functionalities. Briefly, it contains different \emph{Drosophila melanogaster}
...
\paragraph{Retrieving the tutorial data}
First, we load the \Rpackage{curl} library to retrieve the \emph{Drosophila melanogaster}
gene annotation in gtf format, as well as in an Rdata format. Next we retrieve
a couple of BAM formatted files.
...
library(curl)
 ...
 invisible(curl_download(paste0("https://github.com/UPSCb/UPSCb/raw/",
   "master/tutorial/easyRNASeq/gAnnot.rda"),
             "gAnnot.rda"))

   invisible(sapply(c("ACACTG","ACTAGC","ATGGCT","TTGCGA"),function(bam){
...
a regular expression to match the file names (\Rcode{pattern} argument). The pattern used to
...
\item an \Rclass{RNAseq} class object. Useful for performing
  ...
  annotations.
\item as an instance of the \Rclass{RangedSummarizedExperiment} class.
  See section \pref{s:ytc} for additional details.
\end{itemize}
...
chastity filter. Finally, also note the
...
annotation file, please try first to locate and correct them.}
...
count.table <- easyRNASeq(".",
                          ...
                          annotationFile="Dmel-mRNA-exon-r5.52.gff3",
                          gapped=TRUE,
                          count="exons",
                          filenames="gapped.bam")
@
...
\paragraph{CountDataSet, DGEList, RangedSummarizedExperiment}
...
count.table <- easyRNASeq(".",organism="Dmelanogaster",
                          readLength=30L,
                          annotationMethod="rda",
                          annotationFile="gAnnot.rda",
                          count="exons",
...
\Biocpkg{edgeR}) for differential expression analysis.
...
\Rclass{RangedSummarizedExperiment} object. The aim is to have it be the
...
See the \Rclass{RangedSummarizedExperiment} class defined in the
\Biocpkg{SummarizedExperiment} package for more details on last
three accessors used in the example.
\paragraph{}
...
result is returned as an \Rclass{RNAseq} or \Rclass{RangedSummarizedExperiment}
class object, the overlapping genes are flagged by a boolean. To access this
...
and from a \Rclass{RangedSummarizedExperiment} object:
...
function to get a \Rclass{RangedSummarizedExperiment} object back. Mind the
