...
Moreover, if you find \Rpackage{easyRNASeq} useful and apply it in the
...
package. As "gene model" can actually refer to different things depending
on the context, I switched to call these synthetic transcripts as what
we create is a synthetic transcript containing every exons of the gene. 
See the use case: \ref{synth:trx} page \pageref{synth:trx} for how this
can be performed. This example was first introduced at the EMBO 2013 
Practical Course: Analysis of High Throughput Sequencing Data.
...
\Rpackage{easyRNASeq} application note got published in
...
as a simple \textcolor{red}{named vector}. We just use the
...
\textcolor{red}{Moreover, since version $1.3.5$, the 'chr.sizes' can
  ...
  chromosome sizes from the BAM file. Note that this obviously
  does not work if your format is not 'bam'.}
See the additional comments in the use case page \pageref{chrsize}.
\textcolor{red}{In addition, since version $1.3.10$, the
  'filesDirectory' argument defaults to the current directory. This
  one can be looked up using the \Rfunction{getwd()} function.}

\paragraph{Warnings}
\label{p:W}
\textcolor{red}{As you could see when running the previous example,  warnings were
...
As of version $1.3.6$, it is possible to set additional options to
locate the read files. All the options from the list.files base
function are available, \emph{i.e.} one can search for file recursively.
As of version $1.3.7$, variable read lengths is supported. 
The current implementation results in a small, neglictible approximation
of the read counts, due to the likelyhood of an integer overflow. This is
currently investigated and appropriate warnings will be raised if it
were to affect the results.
}
...
using such an annotation file, please try to correct them. For the GFF
format, the example file present in the \Rpackage{RnaSeqTutorial}
package can
...
\label{perf}
\textcolor{red}{
As of version $1.3.8$, it is possible to parallelize some additional
steps in the workflow. So far, only the generation of the geneModel
could be parallelized. As of now, the actual read counting and
summarization can be parallelized too. To achieve this, the 'nbCore'
argument simply has to be set to the number of CPU cores, one wants to
use. There are a number of word of caution though: 
\begin{enumerate}
...
\end{enumerate}
That's it for the cautionary part. And despite these, try it, you'll
get a time boost and that's worth monitoring CPU and RAM for a while.
}
...
\Rpackage{easyRNASeq} package (version $1.5.x$). This in order to
consolidate the kind of objects used for Next-Generation Sequencing in
the Bioconductor repository (rather ambitious, yes). For this, as of
version $1.3.14$, a new function: \Rfunction{count} has been
introduced that supersedes the \Rfunction{easyRNASeq} function. An
example follows, however the parameters -currently alike those of the
\Rfunction{easyRNASeq} - will be refined in order to consolidate and
structure them, so do not rely on them for writing production code.
}
...
\textcolor{red}{
See the \Rpackage{GenomicRange} package \Rclass{SummarizedExperiment}
...
retrieve annotations. \textcolor{red}{Note that the data for this example
...
\Rpackage({biomaRt}. In both use case, we will assume the
\Rpackage{easyRNASeq} library as already been loaded as in:
...
Now we can use the 'gff3' files a our annotation. As we have created synthetic 
...
argument. This is currently being ported to the development version of 
\Rpackage{easyRNASeq}.
...
\textcolor{red}{If you already have a set of bam files resulting of short read
...
in R. Note that the \Rpackage{Rsubread} is only supported on the linux
...
\textcolor{red}{Note that this has generated a number of files that
...
named \textcolor{red}{vector} is sufficient and therefore is easyRNASeq not limited to
...
\textcolor{red}{Alternatively, one can use the \Rfunction{count}
...
\textcolor{red}{Note that as the read length differs between the two
files: $58$ and $76$, the \Robject{readLength} argument was removed
...
\newline
\textcolor{red}{Note that \Rpackage{easyRNASeq} does support some chromosome names
...
chromosome sizes \textcolor{red}{named vector} is as valid.
...
\textcolor{red}{Note that now, providing the chromosome sizes as a list is deprecated.
It has been changed to a named vector, which is more
intuitive. The list was an historical remnant of the RangedData API. 
Note as well that the chr.sizes will be necessary here
