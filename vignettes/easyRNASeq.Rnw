...
\section{Important Note about this developmental version}
\warning{
\Biocpkg{easyRNASeq} is undergoing massive implementation changes. Hence a number
of function are being deprecated and replaced by better alternatives. As the code
is still getting pruned that implies that there are many "deprecated" warnings
currently showing up. These should go away as the re-implementation progresses.
A new introduced function \Rfunction{simpleRNASeq} meant to replace 
\Rfunction{easyRNASeq} is still under active development and hence might not be
fully functional as bugs are still being squashed.}
...
\warning{For better supporting the gff3 format, both feature of type 'mRNA' and 
'exon' (the gff3 third column) are considered. The \textit{ID} key is used in 
both cases to define the transcript and exon IDs, respectively. The 
\textit{Parent} key for the 'mRNA' type is used to define the gene ID.
...
\file{Dmel-mRNA-exon-r5.52.gff3} file used in the next code chunck for an 
example of a gff3 file formatted as the \Rfunction{easyRNASeq} function now 
expects it.
}
