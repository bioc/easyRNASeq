...
Finally, if you find \Rpackage{easyRNASeq} useful and apply it in the
...
\section{Recent changes to the vignette}
...
\paragraph{1.2.3}
Part of that vignette have been recently updated to:
\begin{itemize}
  \item first: enhance its readability.
  \item second: introduce a new \textbf{Use Case} section
\end{itemize}

To make these changes more easily identifiable, they are colored in red.
...
\textcolor{red}{
  ...
  commodity. See the additional comments in the use case page \pageref{chrsize}.
}
...
is described in the section \ref{subsec:diffAnn}, page
\pageref{subsec:diffAnn}. For using a \Rclass{GRangesList}, see the
...
ensure that we return coherent data, we return the counts for every
